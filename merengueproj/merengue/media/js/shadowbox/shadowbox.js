...
*/
        ...
        skin:           {
            ...
            loading:    '<img src="{0}" alt="{1}" />' +
                        '<span><a onclick="Shadowbox.close();">{2}</a></span>',

            counter:    '<b>{0}</b>',

            nav:        '<span id="shadowbox_nav_{0}" class="shadowbox_nav"></span>',

            buttons:    '<span id="shadowbox_nav_buttons" >' +
                        '  <span id="shadowbox_nav_close" class="shadowbox_nav_close"></span>' +
                        '  <span id="shadowbox_nav_previous" class="shadowbox_nav_button"></span>' +
                        '  <span id="shadowbox_counter"></span>' +
                        '  <span id="shadowbox_nav_next" class="shadowbox_nav_button"></span>' +
                        '</span>',
        ...
        ext:     {
            ...
            img_lengths:        ['image_mini', 'image_thumb',  'image_large'],
    ...
    var getPlayerType = function(url){
        ...
        if(RE.img.test(url) || RE.img_lengths.test(url)) {
            return 'img';
        }
                ...
                if(options.handleUnsupported == 'link'){
                    ...
                    switch(match[1]){
                        ...
                        default:
                            ...
                            m = String.format(options.LANG.errors.single,
    ...
    var buildBars = function(){
        ...
        if (link.destino){
            title_i.innerHTML = '<a href="' + link.destino + '">' + link.title + '</a>';
        }
        else{
            title_i.innerHTML = (link.title) ? ('<span>' + link.title + '</span>'): '';
        }
        // empty the toolbar
        ...
        // build the nav
        if(options.displayNav || options.displayClose){
            //tool_i.innerHTML = String.format(options.skin.close, options.LANG.close);
            appendHTML(tool_i, options.skin.buttons);
            
            if(current_gallery.length > 1 && options.displayNav){
                if(options.continuous){
                    // show both
                    appendHTML($('shadowbox_nav_previous'), String.format(options.skin.prev, options.LANG.prev));
                    appendHTML($('shadowbox_counter'), String.format(options.skin.counter, counter));
                    appendHTML($('shadowbox_nav_next'), String.format(options.skin.next, options.LANG.next));
                }else{
                    // not last in the gallery, show the next link
                    if((current_gallery.length - 1) > current){
                        appendHTML($('shadowbox_nav_previous'), String.format(options.skin.next, options.LANG.next));
                    }
                    // not first in the gallery, show the previous link
                    if(current > 0){
                        appendHTML(tool_i, String.format(options.skin.prev, options.LANG.prev));
                    }
                }
            }
        }
        if (link.caption){
            appendHTML(tool_i, String.format(options.skin.caption, link.caption));
        }
    ...
    var loadContent = function(){
        ...
        switch(obj.type){
            case 'img':
                // preload the image
                preloader = new Image();
                SL.addEvent(preloader, 'load', function(){
                ...
                if (preloader) {
                    preloader.src = obj.content + options.appendImg;
                }
            break;
        ...
        if(current_gallery.length > 0){
            ...
            if(!next){
                ...
                if(next != undefined && next.href != undefined && next.type == 'img' ){
                    var preload_next = new Image();
                    preload_next.src = next.href + options.appendImg;
                }
            }
            ...
            if(!prev){
                ...
                if(prev != undefined && prev.href != undefined && prev.type == 'img'){
                    var preload_prev = new Image();
                    preload_prev.src = prev.href + options.appendImg;
                }
            }
        }
    };
    ...
    var adjustWidth = function(width, animate, callback){
        ...
        current_width = width;
        var shadowbox = SL.get('shadowbox');
        shadowbox_inner = SL.get('shadowbox_body_inner');
        if (width < options.minWidth){
            margin = parseInt((options.minWidth - width) / 2) ;
            SL.setStyle(shadowbox_inner, 'margin-left', margin + 'px');
            SL.setStyle(shadowbox_inner, 'margin-right', margin + 'px');
            width = options.minWidth;
        }
        else{
            SL.setStyle(shadowbox_inner, 'margin-left', '0');
            SL.setStyle(shadowbox_inner, 'margin-right', '0');
        }
    ...
    Shadowbox.init = function(opts){
        ...
        RE.img_lengths = new RegExp('\.(' + options.ext.img_lengths.join('|') + ')\s*$', 'i');
        ...
        if(absolute_pos){
            ...
            SL.addEvent(SL.get('shadowbox_container'), 'click', function(e){
                ...
                if(target && target.id && target.id == 'shadowbox_container') Shadowbox.close();
            });
        }
    ...
    Shadowbox.setup = function(links, opts){
        ...
        for(var i = 0, len = links.length; i < len; ++i){
            link = links[i];
            //alert(link.getAttribute('destino'));
    ...
    Shadowbox.buildCacheObj = function(link, opts){
        ...
        var caption_childs = jQuery(link).next(".hidden-caption");
        if (caption_childs.length == 1){
            var caption = caption_childs[0].innerHTML;
        }
        else{
            var caption = '';
        }
        var o = {
            ...
            destino:    link.getAttribute('destino'),
            caption:    caption,
        ...
        var opt, l_opts = ['title', 'type', 'height', 'width', 'gallery', 'destino', 'caption'];
	...
	jQuery('div.panoramic_visor iframe').show();
        activated = false;
    };
