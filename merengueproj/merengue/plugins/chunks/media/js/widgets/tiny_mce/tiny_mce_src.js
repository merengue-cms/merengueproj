...
TinyMCE_Engine.prototype = {
	init : function(settings) {
        ...
        this._def("inplace_edit", false);
        this._def("auto_height", false);
	...
	setupContent : function(editor_id) {
        ...
        if (this.settings['inplace_edit'])
        {
            css_files = document.getElementsByTagName('link');
            for (i=0; i< css_files.length;i++)
            {
                tinyMCE.importCSS(inst.getDoc(), css_files[i].href);
            }
        }
	...
	handleEvent : function(e) {
		...
		switch (e.type) {
			...
			case "mouseleave":
                ...
                if (tinyMCE.settings.inplace_edit){
                    tinyMCE.triggerSave();
                }
				return;
            case "blur":
			...
			case "keyup":
                if (tinyMCE.settings.inplace_edit && (tinyMCE.isSafari || tinyMCE.isOpera)){
                    tinyMCE.triggerSave();
                }
			case "keydown":
				tinyMCE.hideMenus();
...
TinyMCE_Control.prototype = {
    ...
    calculate_offsetHeigth: function(tag){
        ...
        if(tag.offsetHeight!=0 && typeof(tag.offsetHeight) == 'number')
            ...
            for(var i=0; i<tag.childNodes.length;i++) {
                if(tag.childNodes[i].nodeType!=3) {//No sea texto
                    tag_offset += this.calculate_offsetHeigth(tag.childNodes[i]);
                }
	...
	_onAdd : function(replace_element, form_element_name, target_document) {
		...
		if(this.settings['auto_height']) {
		    var text_area = document.getElementById(form_element_name);
		    var form_element_name_view = form_element_name.replace("id", "view");
		    var span = document.getElementById(form_element_name_view);
            if(span) {
		...
		if (dynamicIFrame) {
			...
			var html = tinyMCE.getParam('doctype') + '<html><head xmlns="http://www.w3.org/1999/xhtml"><base href="' + tinyMCE.settings['base_href'] + '" /><title>blank_page</title><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"></head><body style="background:none"></body></html>';
			try {
				if (!this.isHidden())
					this.getDoc().designMode = "on";
